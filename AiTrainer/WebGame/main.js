...
function drawMainMenu() {
    ...
    // Position play button (no logo overlay - menu already has logo)
    const playBtn = gameState.images[ASSETS.playButton];
    if (playBtn) {
        const btnScale = 1.2;
        const btnW = playBtn.width * btnScale;
        const btnH = playBtn.height * btnScale;
        const btnX = canvas.width - btnW - 120;
        const btnY = canvas.height - btnH - 120;
...
function drawBattle() {
    ...
    // Debug: Draw scene name on screen
    ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
    ctx.fillRect(10, canvas.height - 30, 300, 25);
    ctx.fillStyle = '#00FF00';
    ctx.font = '12px monospace';
    ctx.fillText(`Scene: ${gameState.scene} | SlideComplete: ${battle.slideComplete}`, 15, canvas.height - 10);

    const playerBotX = battle.slideComplete ? 450 : battle.playerBotX;
    const enemyBotX = battle.slideComplete ? 850 : battle.enemyBotX;

    const tabiaImg = gameState.images[battle.playerSprites.tabia];
    if (tabiaImg) {
        const tabiaScale = 0.5;
        const tabiaW = tabiaImg.width * tabiaScale;
        const tabiaH = tabiaImg.height * tabiaScale;
        ctx.drawImage(tabiaImg, 200, canvas.height - tabiaH - 80, tabiaW, tabiaH);
    }
    ...
    if (botImg) {
        ...
        ctx.drawImage(botImg, playerBotX, canvas.height - botH - 80, botW, botH);
    }

    const enemyImg = gameState.images[battle.enemySprite];
    if (enemyImg) {
        const enemyScale = 0.4;
        const enemyW = enemyImg.width * enemyScale;
        const enemyH = enemyImg.height * enemyScale;
        ctx.drawImage(enemyImg, enemyBotX, canvas.height - enemyH - 200, enemyW, enemyH);
    }
...
function drawStatPanelsWithIcons() {
    ...
    if (topLeftBox) {
        ctx.drawImage(topLeftBox, 10, 10, 300, 160);

        const iconMarquee = gameState.images[ASSETS.iconMarquee];
        if (iconMarquee) {
            ctx.drawImage(iconMarquee, 20, 20, 32, 32);
        }

        drawPixelText(battle.player.name, 60, 42, 14, '#FFD700', 'left', true);

        drawPixelText('HITS', 20, 75, 12, '#FFFFFF', 'left', false);
        drawPixelText(`${battle.player.hits} / 9`, 80, 75, 16, '#00FF00', 'left', true);

        drawPixelText('HEAT', 20, 100, 12, '#FFFFFF', 'left', false);
        const heatPercent = Math.min(battle.player.heat / battle.player.stats.power, 1);
        const hpBarWidth = 200;
        const hpBarHeight = 12;
        ctx.fillStyle = '#222';
        ctx.fillRect(60, 85, hpBarWidth, hpBarHeight);
        const heatColor = heatPercent > 0.8 ? '#FF0000' : heatPercent > 0.5 ? '#FF8C00' : '#FFFF00';
        ctx.fillStyle = heatColor;
        ctx.fillRect(60, 85, hpBarWidth * heatPercent, hpBarHeight);
        ...
        if (iconGpu) ctx.drawImage(iconGpu, 20, 120, 20, 20);
        drawPixelText(`${battle.player.stats.compute}`, 45, 135, 11, '#CCC', 'left', false);

        if (iconCpu) ctx.drawImage(iconCpu, 100, 120, 20, 20);
        drawPixelText(`${battle.player.stats.logic}`, 125, 135, 11, '#CCC', 'left', false);

        if (iconRam) ctx.drawImage(iconRam, 180, 120, 20, 20);
        drawPixelText(`${battle.player.stats.io}`, 205, 135, 11, '#CCC', 'left', false);

        drawPixelText(`NET:${battle.player.stats.network}`, 20, 157, 10, '#CCC', 'left', false);
        drawPixelText(`MEM:${battle.player.stats.memory}`, 120, 157, 10, '#CCC', 'left', false);
    }

    const bottomRightBox = gameState.images[ASSETS.uiBoxBottomRight];
    if (bottomRightBox) {
        ctx.drawImage(bottomRightBox, canvas.width - 310, 10, 300, 160);

        const iconEnemy = gameState.images[ASSETS.iconEnemy];
        if (iconEnemy) {
            ctx.drawImage(iconEnemy, canvas.width - 295, 20, 32, 32);
        }

        drawPixelText(battle.enemy.name, canvas.width - 255, 42, 14, '#FF6B6B', 'left', true);

        drawPixelText('HITS', canvas.width - 295, 75, 12, '#FFFFFF', 'left', false);
        drawPixelText(`${battle.enemy.hits} / 9`, canvas.width - 225, 75, 16, '#FF6B6B', 'left', true);

        drawPixelText('HEAT', canvas.width - 295, 100, 12, '#FFFFFF', 'left', false);
        const heatPercent = Math.min(battle.enemy.heat / battle.enemy.stats.power, 1);
        ctx.fillStyle = '#222';
        ctx.fillRect(canvas.width - 245, 85, hpBarWidth, hpBarHeight);
        const heatColor = heatPercent > 0.8 ? '#FF0000' : heatPercent > 0.5 ? '#FF8C00' : '#FFFF00';
        ctx.fillStyle = heatColor;
        ctx.fillRect(canvas.width - 245, 85, hpBarWidth * heatPercent, hpBarHeight);

        drawPixelText(`GPU:${battle.enemy.stats.compute}`, canvas.width - 295, 125, 10, '#CCC', 'left', false);
        drawPixelText(`CPU:${battle.enemy.stats.logic}`, canvas.width - 295, 142, 10, '#CCC', 'left', false);
        drawPixelText(`I/O:${battle.enemy.stats.io}`, canvas.width - 295, 159, 10, '#CCC', 'left', false);
        drawPixelText(`NET:${battle.enemy.stats.network}`, canvas.width - 180, 125, 10, '#CCC', 'left', false);
        drawPixelText(`MEM:${battle.enemy.stats.memory}`, canvas.width - 180, 142, 10, '#CCC', 'left', false);
    }
}
...
function drawCommandInterface() {
    ...
    drawPixelText('COMMAND INTERFACE', canvas.width / 2, panelY + 30, 18, '#4ECDC4', 'center', true);

    const btnWidth = 220;
    const btnHeight = 40;
    const btnSpacing = 20;
    const startX = (canvas.width - (btnWidth * 4 + btnSpacing * 3)) / 2;
    ...
    commands.forEach((cmd, i) => {
        ...
        ctx.fillStyle = cmd.enabled ? 'rgba(78, 205, 196, 0.5)' : 'rgba(100, 100, 100, 0.3)';
        ctx.fillRect(btnX, btnY, btnWidth, btnHeight);
        ctx.strokeStyle = cmd.enabled ? '#4ECDC4' : '#666';
        ctx.lineWidth = 3;
        ctx.strokeRect(btnX, btnY, btnWidth, btnHeight);

        drawPixelText(cmd.text, btnX + btnWidth / 2, btnY + 28, 13, cmd.enabled ? '#FFFFFF' : '#666', 'center', true);
...
function drawMoveSelectionSidePanel() {
    ...
    drawPixelText('SELECT FUNCTION', panelX + panelWidth / 2, 45, 16, '#4ECDC4', 'center', true);
    ...
    moves.forEach((move, i) => {
        ...
        drawPixelText(move.name, btnX + 15, btnY + 28, 14, '#FFD700', 'left', true);
        drawPixelText(`[${move.slot}]`, btnX + 15, btnY + 50, 9, '#888', 'left', false);
        drawPixelText(move.description, btnX + 15, btnY + 70, 9, '#CCC', 'left', false);

        drawPixelText(`DMG:${move.compute}`, btnX + btnWidth - 15, btnY + 35, 10, '#FF6B6B', 'right', false);
        drawPixelText(`HEAT:${move.heat > 0 ? '+' : ''}${move.heat}`, btnX + btnWidth - 15, btnY + 55, 10, '#FF8C00', 'right', false);
        drawPixelText(`ACC:${move.accuracy}%`, btnX + btnWidth - 15, btnY + 75, 10, '#4ECDC4', 'right', false);
    ...
    drawPixelText('BACK', backBtnX + backBtnW / 2, backBtnY + 30, 13, '#FFFFFF', 'center', true);
...
function drawPostBattle(winner) {
    ...
    if (resultSprite) {
        ...
        ctx.drawImage(resultSprite, (canvas.width - w) / 2, 80, w, h);
    }

    drawPixelText('BATTLE SUMMARY', canvas.width / 2, 250, 18, '#FFD700', 'center', true);

    const summaryX = canvas.width / 2 - 300;
    let summaryY = 310;

    drawPixelText(`Data Packets: ${battle.rewards.dataPackets}`, summaryX, summaryY, 14, '#CCC', 'left', false);
    summaryY += 40;
    drawPixelText(`XP Gained: ${battle.rewards.xp}`, summaryX, summaryY, 14, '#CCC', 'left', false);
    summaryY += 40;
    drawPixelText(`PMP: +${battle.rewards.pmp}`, summaryX, summaryY, 14, '#CCC', 'left', false);
    summaryY += 40;
    drawPixelText(`Parts: ${battle.rewards.partsGained.length > 0 ? battle.rewards.partsGained.join(', ') : 'None'}`, summaryX, summaryY, 12, '#CCC', 'left', false);
    summaryY += 60;

    drawPixelText(`Player: ${battle.player.integrity}%  Enemy: ${battle.enemy.integrity}%`, summaryX, summaryY, 11, '#888', 'left', false);

    drawPixelText('Click to return to menu', canvas.width / 2, canvas.height - 60, 16, '#4ECDC4', 'center', true);
}
...
function animateSlideIn() {
    ...
    function slide() {
        if (battle.playerBotX < 450) battle.playerBotX += 20;
        if (battle.enemyBotX > 850) battle.enemyBotX -= 20;

        if (battle.playerBotX >= 450 && battle.enemyBotX <= 850) {
            battle.slideComplete = true;
            console.log('Slide complete! Transitioning to TURN_SELECT');
            battle.addLog('=== BATTLE START ===');
            battle.addLog(`${battle.player.name} vs ${battle.enemy.name}`);

            setTimeout(() => {
                gameState.scene = 'TURN_SELECT';
                console.log('Scene now:', gameState.scene);
            }, 500);
        } else {
